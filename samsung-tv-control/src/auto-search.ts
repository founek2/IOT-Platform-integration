import * as dgram from 'npm:dgram'
import { Client, SsdpHeaders } from 'npm:node-ssdp'
...
class AutoSearch {
  ...
  public deviceUpdate(headers: SsdpHeaders, _: number, rinfo: dgram.RemoteInfo) {
    ...
    request.get({ url: `http://${rinfo.address}:8001/api/v2/` }, (err: Error, res: any, body: string) => {
