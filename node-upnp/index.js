...
import concat from 'npm:concat-stream';
import address from 'npm:network-address';
import version from "./package.json" with { type: "json" };

const DEFAULT_USER_AGENT = `${os.platform()}/${os.release()} UPnP/1.1 ${version.name
  }/${version.version}`;
...
import { createSOAPAction } from './request.js';
import { resolveService } from './util.js';
import error from './error.js';

export class UPnPClient {
  constructor({ url, userAgent = DEFAULT_USER_AGENT, eventsServerPort = 8085 }) {
    this.url = new URL(url);
    this.deviceDescription = null;
    this.serviceDescriptions = {};
    this.eventsServer = null;
    this.subscriptions = {};
    this.eventsServerPort = eventsServerPort;
  ...
  async getEventsServer() {
    ...
    if (!this.eventsServer.listening) {
      await new Promise(resolve => {
        this.eventsServer.listen(this.eventsServerPort, address.ipv4(), resolve);
      });
    }
